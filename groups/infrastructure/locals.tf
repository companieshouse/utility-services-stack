locals {
  ...
  heritage_app_subnet_pattern     = local.stack_secrets["heritage_app_subnet_pattern"]
  ...
  felixvalidator_ingress_cidrs    = concat(local.ingress_cidrs_private, local.heritage_application_cidrs)
  ...
  heritage_application_cidrs      = [for subnet in data.aws_subnet.heritage_application : subnet.cidr_block]
